...
const endpointHandler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (!api_key) {
        res.status(401).json({ error: "You do not have a valid API key" })
    }
    ...
    if (!user) {
        res.status(401).json({ error: "No user with that API key exists" })
    }
    ...
    if (!item) {
        res.status(403).json({ error: "You do not have a subscription" })
    }
